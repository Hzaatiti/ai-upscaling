...
## Exercise 2, Use the NYU theme (10 min)

1. In your `slides.md`, change the frontmatter to use the NYU theme:
   ```yaml
   ---
   theme: nyu
   title: My NYU-flavored Deck
   author: Your name
   ---
   ```
2. Inside this repo the theme is wired up for you via a `file:` dependency on the `theme/` folder, no install command needed. In your own project elsewhere, copy the `theme/` folder next to your deck and depend on it with `"slidev-theme-nyu": "file:../theme"`.
3. Add a cover slide with `layout: cover`, a section divider with `layout: section`, and a `<NyuCallout label="Tip">` somewhere.
...
- Look at `theme/example.md` in this repo to see every layout and component live.
- Open a PR adding a new layout you wish the theme had.
