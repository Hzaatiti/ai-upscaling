 ...
 *        - package.json   (name = nyu-wsNN-<slug>, theme via file: to the
 *                          in-repo theme/ package)
 ...
 *
 * Requirements: none beyond this repo. The Slidev theme (slidev-theme-nyu)
 * is vendored at theme/ and each workshop consumes it via a relative
 * `file:../../theme` dependency.
 *
...
// In-repo Slidev theme package. Directory layout:
//   <root>/
//   ├── theme/            <-- slidev-theme-nyu (THEME_DIR)
//   └── workshops/NN-.../ <-- each deck depends on it via file:../../theme
const THEME_DIR = resolve(root, 'theme')
const THEME_PKG_REL_FROM_WORKSHOP = '../../theme'

if (!existsSync(THEME_DIR)) {
  console.error(`Cannot find the theme package at ${THEME_DIR}`)
  console.error('The theme/ folder should be part of this repo; restore it from git.')
  process.exit(1)
}
...
const titleCase = slug
  ...
  .join(' ')
const pkgName = `nyu-ws${numStr}-${slug}`
...
// is the canonical copy inside the theme package.
const brandSrc = join(THEME_DIR, 'public', 'brand', 'nyuad-logo.png')
  ...
  JSON.stringify(
    {
      ...
      version: '0.1.0',
      private: true,
      description: `NYU Upscaling workshop, ${titleCase}.`,
      scripts: {
        dev: 'slidev --open',
        build: 'slidev build',
        export: 'slidev export',
        'export:pdf': 'slidev export --format pdf',
        'export:pptx': 'slidev export --format pptx --per-slide --no-with-clicks --timeout 90000',
      },
      dependencies: {
        // file: protocol, symlinks to the in-repo theme package.
        // Equivalent to `link:` in pnpm but works on both npm and pnpm,
        // so consumer setups using either tool work without changes.
        'slidev-theme-nyu': `file:${THEME_PKG_REL_FROM_WORKSHOP}`,
      },
      devDependencies: {
        '@slidev/cli': '^0.49.0',
        '@slidev/types': '^0.49.0',
...
info: |
  NYU Upscaling Workshop ${numStr}, ${titleCase}.
...
::eyebrow::
<span class="nyu-tag nyu-tag--accent">Workshop ${numStr}</span>
...
::meta::
Questions? Reach out to the workshop maintainers.
`,
)
...
- \`public/brand/\`, NYUAD lockup (mirrored from theme/public/brand/)
- \`exercises/\`, hands-on exercises that pair with the deck
- \`components/\`, workshop-specific Vue components (optional)

See \`theme/README.md\` at the repo root for the full theme reference (layouts, components, tokens).
`,
)
